...
def boundary_conditions(c_array, n_x, n_y, dt, N_flux):
    """Set the boundary condition values with nitrate flux."""
    c_array[0,:] = 0
    c_array[1,:] = 0
    c_array[2,:] = 0
    c_array[3,:] = 0
...
#     n_time, n_grid = map(int, args)
    ...
    c_now = c_vals[0,:,:]
    c_now = boundary_conditions(c_now, n_x, n_y, dt, N_flux)
    c_vals[0,:,:] = c_now
    ...
    for ts in np.arange(0, n_time):
        ...
        c_next = boundary_conditions(c_next, n_x, n_y, dt, N_flux)
